      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <Link href={profileUrl}>
              <h3 className="text-base font-bold text-slate-800 hover:text-[var(--primary)] transition-colors">
        ...
        {appointment.meetingType === 'VIDEO_CALL' && appointment.meetingLink && isAccepted && (
          <a
            ...
            className="inline-flex items-center justify-center gap-1.5 px-4 py-2 text-xs font-semibold text-white bg-indigo-600 hover:bg-indigo-700 rounded-xl transition-all shadow-2xs flex-shrink-0"
          >
            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div className="flex items-center gap-2 ml-auto">
          {!isPsychologist && isScheduled && onAccept && (
            <button
              ...
              disabled={isMutating}
              className="inline-flex items-center gap-1.5 px-3.5 py-1.5 bg-emerald-600 hover:bg-emerald-700 text-white text-xs font-semibold rounded-xl transition-all shadow-2xs disabled:opacity-50 cursor-pointer"
            >
          ...
          {(isScheduled || isAccepted) && onCancel && (
            ...
            >
              ...
              {!isPsychologist && isScheduled ? 'Recusar' : 'Cancelar'}
            </button>
          )}
        </div>
      </div>
