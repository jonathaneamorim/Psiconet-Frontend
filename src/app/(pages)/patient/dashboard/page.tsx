    ...
    <div className="w-full min-h-screen bg-slate-50 py-24 sm:py-28 px-4 sm:px-8">
      <div className="max-w-6xl mx-auto flex flex-col gap-8">
        ...
        {pendingAppointments.length > 0 && (
          <div className="bg-amber-50 border border-amber-200/90 rounded-3xl p-5 shadow-2xs flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div className="flex items-center gap-3.5">
              ...
              <div>
                <h3 className="text-sm font-bold text-amber-900">
                  Você tem {pendingAppointments.length} agendamento(s) aguardando sua confirmação
                </h3>
                <p className="text-xs text-amber-700 mt-0.5">
                  Acesse sua agenda para confirmar seu horário com seu psicólogo.
                </p>
              </div>
            </div>
            ...
            <Link
              href={ROUTES.PATIENT_APPOINTMENTS}
              className="px-4 py-2 bg-amber-600 hover:bg-amber-700 text-white text-xs font-semibold rounded-xl transition-colors shadow-2xs self-start sm:self-center"
            >
              Confirmar Horários →
            </Link>
          </div>
        ...
        {nextAppointment && (
          <div className="bg-white rounded-3xl border border-slate-200/80 p-6 sm:p-7 shadow-2xs flex flex-col md:flex-row md:items-center justify-between gap-6">
            ...
            <div className="flex flex-wrap items-center gap-3 flex-shrink-0">
              {nextAppointment.meetingType === 'VIDEO_CALL' && nextAppointment.meetingLink && nextAppointment.status === 'ACCEPTED' && (
                <a
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
          ...
          <div className="bg-white p-5 rounded-2xl border border-slate-200/80 shadow-2xs flex flex-col justify-between gap-3">
            ...
            <p className="text-2xl sm:text-3xl font-bold text-amber-700">{pendingAppointments.length}</p>
          </div>
                ...
                <div className="flex flex-col gap-3">
                  {connections.map((conn) => (
                    ...
                    >
                      ...
                      <Link
                        href={`/patient/profile/${conn.user.id}`}
