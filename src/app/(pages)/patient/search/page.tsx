import { searchPsychologistsAction } from '@/actions/profile';
import { ProfileCard } from '@/components/Organism/ProfileCard';
import { Pagination } from '@/components/Molecules/Pagination';

export default async function PatientSearchPage(props: {
  ...
  let data, error;
  if (isSearchActive) {
    const res = await searchPsychologistsAction(query, page, 10);
    data = res.data;
    error = res.error;
  }
    ...
    <div className="w-full min-h-screen bg-slate-50 py-24 sm:py-32 px-4 sm:px-8">
      <div className="max-w-4xl mx-auto">
        <div className="flex flex-col gap-2 mb-8">
          <h1 className="text-3xl font-bold text-slate-800">Buscar Psicólogos</h1>
          <p className="text-slate-600">Encontre o profissional ideal para você e conecte-se.</p>
        </div>
        ...
        {error && isSearchActive ? (
          ...
          <div className="bg-white p-12 rounded-3xl shadow-sm border border-slate-200 text-center flex flex-col items-center justify-center">
            ...
            <p className="text-slate-500 mt-2">Digite um nome ou cidade acima para encontrar psicólogos.</p>
          </div>
          ...
          <div className="bg-white p-12 rounded-3xl shadow-sm border border-slate-200 text-center flex flex-col items-center justify-center">
            ...
            <h3 className="text-lg font-bold text-slate-700">Nenhum psicólogo encontrado</h3>
            <p className="text-slate-500 mt-2">Tente buscar por outros termos ou especialidades.</p>
          </div>
        ) : (
          <div className="flex flex-col gap-4">
            {contentList.map((psychologist: any) => (
              <ProfileCard key={psychologist.id} profile={psychologist} profileType="psychologist" />
            ))}
